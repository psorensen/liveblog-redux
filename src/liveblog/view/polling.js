...
export function poll( container, config ) {
	...
	const url = `${ config.restUrl }?since=${
		state.lastModified
	}&per_page=${ perPage }&_=${ Date.now() }`;
	...
	fetch( url )
			...
			if ( wasInitialSync && data.updates && data.updates.length > 0 ) {
				...
				if ( state.hasMore ) {
					container.appendChild(
						getOrCreateLoadMoreButton( container )
					);
				}
			} else if (
				wasInitialSync &&
		...
		.catch( () => {
			state.backoff = Math.min(
				state.backoff
					? Math.min( state.backoff * 2, MAX_BACKOFF )
					: MIN_BACKOFF,
				MAX_BACKOFF
			);
		} )
